...
import {
  duplaLabelByPair, duplaOfRoundLocal, koTeamsForRound, koTitle, winnerOf,
} from '../../data/domain';
...
function FinalBox({ week, koKey, title, serie, duplaA, duplaB, champLabel }: FinalBoxProps) {
  const { players, weeks, canEdit, setKnockoutScore } = useSeason();
  const weekData = weeks[week];
  const assigned = weekData?.assignedRound ?? null;
  const s = weekData?.knockout[koKey] ?? { h: '', a: '' };
  ...
  if (duplaA === null || duplaB === null) {
    return (
      <div className={`ko-box ko-final ${serie}`}>
        <h5>{title}</h5>
        <div className="note" style={{ margin: 0 }}>Aguardando resultado das semifinais…</div>
      </div>
    );
  }
  ...
  const champPair = winner !== null ? duplaOfRoundLocal(assigned, winner) : null;
  ...
  return (
    <div className={`ko-box ko-final ${serie}`}>
      ...
      {champPair && (
        <div className={`champion-badge ${serie}`}>🏆 {champLabel}: {duplaLabelByPair(champPair, weekData, players)}</div>
      )}
    </div>
  );
}
    ...
    <div className="card" id="knockoutCard">
      ...
      <div className="serie-block">
        ...
        <div className="bracket">
          ...
          <FinalBox week={week} koKey="finalG" title="Final Série Ouro" serie="ouro" duplaA={wG1} duplaB={wG2} champLabel="Campeã Série Ouro" />
        </div>
      </div>
      ...
      <div className="serie-block">
        ...
        <div className="bracket">
          ...
          <FinalBox week={week} koKey="finalS" title="Final Série Prata" serie="prata" duplaA={wS1} duplaB={wS2} champLabel="Campeã Série Prata" />
        </div>
      </div>
    </div>
  );
